...
void HelloTriangleApp::createDescriptionSetLayout(){
    ...
    if(vkCreateDescriptorSetLayout(device, &createInfo , nullptr, &descSetLayout) != VK_SUCCESS){
        throw std::runtime_error("create descriptor setlayout failed!");        
    }
}
...
void HelloTriangleApp::createVertexBuffer(){
    ...
    VkDeviceSize buffSize = sizeof(Vertex) * vertexData.size();
    ...
    //fill vertex data
    void *data;
    vkMapMemory(device, stageMemory, 0, buffSize, 0, &data);
        memcpy(data, vertexData.data(), (size_t)buffSize);
    ...
    copyBuffer(stageBuffer, vertexBuffer, buffSize);
...
void HelloTriangleApp::createIndexBuffer(){
    std::cout << "Create IndexBuffer" << std::endl;

    VkDeviceSize buffSize = sizeof(uint16_t) * indices.size();
    ...
    copyBuffer(stageBuffer, indexBuffer, buffSize);
...
void HelloTriangleApp::createUniformBuffers(){
    ...
    uniformMemorys.resize(MAX_FRAMES_IN_FLIGHT);
    ...
    for(size_t i = 0;i < MAX_FRAMES_IN_FLIGHT; i++){
        ...
        createBuffer(buffSize,
            ...
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            uniformBuffers[i], uniformMemorys[i]);
        
        vkMapMemory(device, uniformMemorys[i], 0, buffSize, 0, &uniformMapped[i]);
    }//end for i
}
...
void HelloTriangleApp::createDescriptorPool(){
    ...
    createPoolInfo.poolSizeCount = 1;
    createPoolInfo.pPoolSizes = &poolSize;
    createPoolInfo.maxSets = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);

    if(vkCreateDescriptorPool(device, &createPoolInfo, nullptr, &descPool) != VK_SUCCESS){
...
void HelloTriangleApp::updateUniformBuffer(uint32_t currentFrameIndex){
    ...
    ubo.projMat = glm::perspective(glm::radians(45.0f), ratio, 0.1f , 10.0f);
    ubo.projMat[1][1] *= -1;

    memcpy(uniformMapped[currentFrameIndex], &ubo, sizeof(UniformBufferObject));
}
...
void HelloTriangleApp::recordRenderCmds(uint32_t imageIndex , VkCommandBuffer cmdBuffer){
    ...
    vkCmdBeginRenderPass(cmdBuffer, &passBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
        ...
        vkCmdBindDescriptorSets(cmdBuffer, 
            ...
            pipelineLayout, 0, 1, &descSets[currentFrameIndex], 0, nullptr);
        vkCmdDrawIndexed(cmdBuffer, static_cast<uint32_t>(indices.size()), 
            1, 0, 0, 0);
...
void HelloTriangleApp::copyBuffer(VkBuffer srcBuf, VkBuffer dstBuf, VkDeviceSize size){
    ...
    regions.srcOffset = 0;
    regions.dstOffset = 0;
    vkCmdCopyBuffer(transferCmdBuffer, srcBuf, dstBuf, 1, &regions);
    ...
    vkFreeCommandBuffers(device, cmdPool, 1, &transferCmdBuffer);
}
...
void HelloTriangleApp::dispose(){
    ...
    for(size_t i = 0 ; i < MAX_FRAMES_IN_FLIGHT ; i++){
        vkFreeMemory(device, uniformMemorys[i], nullptr);
